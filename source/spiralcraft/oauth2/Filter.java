  ...
  extends FocusFilter<Session>
{
  ...
  public URI startAuthSequence(String callbackURIString)
  {
    ...
    { 
      callbackQuery=callbackURI.getRawQuery();
      VariableMap params=VariableMap.fromUrlEncodedString(callbackQuery);
  ...
  { 
    ...
    String authCode=query.getValue("code");
    String authVerifier=query.getValue("state");
    
    if (debug)
    { log.fine("Auth: "+authCode+":"+authVerifier);
    }
    ...
    try
    { session.completeAuthSequence(authCode,authVerifier,null);
    }
    catch (IOException x)
